...
class LLaMA_adapter(nn.Module):
    ...
    def encode_audio(self, x):
        inputs = self.mert_processor(load_audio(x, target_sr=self.mert_processor.sampling_rate)[0],
                                     sampling_rate=self.mert_processor.sampling_rate, return_tensors="pt").to(self.device)
        with torch.no_grad():
            outputs = mert_model(**inputs, output_hidden_states=True)
        all_layer_hidden_states = torch.stack(outputs.hidden_states).squeeze()
        x = all_layer_hidden_states.mean(-2).unsqueeze(0)
        return self.mert_agg(x).squeeze()
    ...
    def forward_visual(self, inputs, cache_size=10, cache_t=20, cache_weight=0.5):
        ...
        for input_type, (input, input_weight) in inputs.items():
            ...
            elif input_type.lower() in ['audio']:
    ...
    def forward(self, tokens, labels, imgs):
        visual_feats = self.forward_visual({'Audio': [imgs, 1]})
