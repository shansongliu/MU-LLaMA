...
parser.add_argument(
    "--audio_paths", nargs='+', required=True, type=str,
    help="Paths to the input music file",
)
parser.add_argument(
    "--questions", nargs='+', default="Describe the Audio", type=str,
    help="Questions to ask the model",
)
...
if len(args.questions) != len(args.audio_paths):
    if len(args.questions) == 1:
        args.questions = args.questions * len(args.audio_paths)
    elif len(args.audio_paths) == 1:
        args.audio_paths = args.audio_paths * len(args.questions)
    else:
        print("Number of Questions must match Number of Music files")
        exit(0)
...
def multimodal_generate(
    ...
    inputs = {}
    audio = load_and_transform_audio_data(audio_paths)
    inputs['Audio'] = [audio, audio_weight]
    prompts = [llama.format_prompt(prompt) for prompt in prompts]
    ...
    text_outputs = [result.strip() for result in results]
    return text_outputs


outputs = multimodal_generate(args.audio_paths, 1, args.questions, 100, 20.0, 0.0, 512, 0.6, 0.8)

for i in range(len(args.audio_paths)):
    print()
    print(f"Audio File: {args.audio_path[i]}")
    print(f"Q: {args.questions[i]}")
    print(f"A: {outputs[i]}")
